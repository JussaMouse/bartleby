...
- **Node.js 22+**: `node --version` → v22.x.x
...
- [ ] `help` → shows help text (pattern match)
- [ ] `show next actions` → shows list (pattern match)
- [ ] `add task buy milk @errands` → creates task (pattern match)
- [ ] `capture call dentist` → adds to inbox (pattern match)
- [ ] `done 1` → marks first task complete (pattern match)
- [ ] `waiting for` → shows waiting items (pattern match)

**Calendar:**
- [ ] `today` → shows today's events (pattern match)
- [ ] `calendar` → shows upcoming (pattern match)
- [ ] `add event meeting tomorrow at 3pm` → creates event (pattern match)

**Contacts:**
- [ ] `add contact Sarah, email sarah@example.com` → creates contact
- [ ] `find sarah` → finds contact

**Memory:**
- [ ] `what do you know about me` → shows profile (pattern match)
- [ ] `I prefer morning meetings` → stores preference
- [ ] Exit and restart → proactive opener shows relevant info

**Shed (Reference Library):**
- [ ] `ingest notes.md` → ingests document, shows chunk count
- [ ] `list sources` → shows ingested documents
- [ ] `ask shed about <topic>` → RAG query returns relevant answer with citations

**Scheduler (Reminders):**
- [ ] `remind me to call dentist at 3pm` → creates reminder
- [ ] `show reminders` → lists scheduled tasks
- [ ] `daily at 9am check email` → creates recurring reminder
- [ ] `cancel reminder 1` → cancels the reminder
- [ ] Wait for reminder time → notification appears

**System:**
- [ ] `status` → shows system status
- [ ] `weather` → shows weather (if configured)
- [ ] `quit` → exits cleanly
...
- [ ] `email Sarah about our meeting tomorrow` → Should:
  1. Show "🤔 This looks like a complex request..."
  2. Look up Sarah's contact
  3. Check tomorrow's calendar
  4. Draft email (or ask for confirmation)

- [ ] `compare my tasks with my calendar for this week` → Should:
  1. Get active tasks
  2. Get this week's events
  3. Synthesize comparison

**Edge Cases:**
- [ ] Empty input → shows prompt again
- [ ] Gibberish → LLM tries to help or says "try help"
- [ ] Ctrl+C → exits cleanly
- [ ] Router model down → Falls back to heuristics
...
Should show: `LLM: Router ✓ | Fast ✓ | Thinking ✓`
...
- **5-layer routing**:
  1. Complexity classification (Router model → SIMPLE or COMPLEX)
...
- **Bidirectional sync**: SQLite ↔ Markdown files
...
│       ↓
│   Plan → Tool 1 → Tool 2 → ... → Synthesize
│
└─→ SIMPLE
        ↓
    Command Router (Pattern → Keyword → Semantic)
        ↓
    ├─→ Match → Execute tool directly (~10-100ms)
    └─→ No match → Fast Model (single tool, ~1-2s)
```
...
4. Full cron expression support for scheduler (add `cron-parser` library)
5. Additional document formats (EPUB, DOCX)
